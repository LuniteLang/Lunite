...
use crate::ast::{Block, Expression, FunctionDecl, Item, Program, Statement, Type, StructDecl, EnumDecl, ImplDecl, Visibility, StructLayout};
...
impl Scope {
    ...
    pub fn with_parent(parent: Rc<RefCell<Scope>>) -> Self { Self { symbols: HashMap::new(), parent: Some(parent) } }
    pub fn insert(&mut self, name: String, symbol: Symbol) { self.symbols.insert(name, symbol); }
    pub fn lookup(&self, name: &str) -> Option<Symbol> {
        if let Some(sym) = self.symbols.get(name) { return Some(sym.clone()); }
        if let Some(parent) = &self.parent { return parent.borrow().lookup(name); }
        None
    }
}
...
#[derive(Debug, Clone)]
pub enum TStatement {
    Let { name: String, var_type: Type, value: Option<TExpression> },
    Return(Option<TExpression>),
    Expr(TExpression),
    Assign { lvalue: TExpression, rvalue: TExpression },
    If { condition: TExpression, then_block: TBlock, else_block: Option<TBlock> },
    While { condition: TExpression, body: TBlock },
    Region { body: TBlock },
    Block(TBlock),
    Throw { value: TExpression },
    Spawn(TExpression),
    TryCatch { try_block: TBlock, catch_variable: String, catch_block: TBlock },
    When { subject: TExpression, arms: Vec<TWhenArm> },
}
...
pub enum TWhenPattern {
    Literal(TExpression),
    EnumVariant { enum_name: String, variant_name: String, binding: Option<String>, tag: u32 },
    Else,
}
...
pub enum TExpressionKind {
    ...
    String(String),
    Identifier(String),
    StructInit { name: String, fields: Vec<(String, TExpression)> },
    EnumInit { enum_name: String, variant_name: String, value: Option<Box<TExpression>>, tag: u32 },
    Call { function: String, args: Vec<TExpression> },
    MemberAccess { object: Box<TExpression>, field: String },
    ArrayRepeat { value: Box<TExpression>, size: usize },
    ArrayLiteral { elements: Vec<TExpression> },
    Index { array: Box<TExpression>, index: Box<TExpression> },
    Binary { left: Box<TExpression>, operator: TokenKind, right: Box<TExpression> },
    Unary { operator: TokenKind, right: Box<TExpression> },
    Cast { expression: Box<TExpression>, target_type: Type },
    Comptime { body: TBlock },
    Try { expression: Box<TExpression> },
    Sizeof(Type),
}
...
impl SemanticAnalyzer {
    ...
    fn cur_error(&self, msg: &str) -> CompileError { CompileError { message: msg.into(), line: 0, column: 0 } }
    fn enter_scope(&mut self) { let new_scope = Scope::with_parent(self.current_scope.clone()); self.current_scope = Rc::new(RefCell::new(new_scope)); }
    fn exit_scope(&mut self) { let parent = self.current_scope.borrow().parent.clone(); if let Some(p) = parent { self.current_scope = p; } }

    fn mangle_name(&self, base_name: &str, type_args: &[Type]) -> String {
        let suffix = type_args.iter().map(|t| {
            match t {
                ...
                Type::Array(inner, size) => format!("arr_{}_{}", self.mangle_name("", &[*inner.clone()]), size),
        ...
        if base_name.is_empty() { return suffix; }
    ...
    fn substitute_type(&self, typ: &Type, mapping: &HashMap<String, Type>) -> Type {
        match typ {
            Type::Custom(name, args) => {
                if let Some(sub) = mapping.get(name) { return sub.clone(); }
                let new_args: Vec<Type> = args.iter().map(|t| self.substitute_type(t, mapping)).collect();
                Type::Custom(name.clone(), new_args)
            }
            Type::Array(inner, size) => Type::Array(Box::new(self.substitute_type(inner, mapping)), *size),
            Type::Pointer(inner) => Type::Pointer(Box::new(self.substitute_type(inner, mapping))),
            Type::Function(params, ret) => {
                let new_params = params.iter().map(|p| self.substitute_type(p, mapping)).collect();
    ...
    fn instantiate_struct(&mut self, base_name: &str, type_args: &[Type]) -> Result<String, CompileError> {
        ...
        for (i, param) in origin_decl.generic_params.iter().enumerate() { mapping.insert(param.clone(), type_args[i].clone()); }
        ...
        self.structs.insert(mangled_name.clone(), (new_decl.clone(), origin_mod.clone()));
        ...
        if let Some((impl_decl, _)) = self.generic_impls.get(original_name).cloned() {
            for method in impl_decl.methods { self.instantiate_function_from_decl(method, &mapping, &mangled_name, &origin_mod)?; }
        }
        Ok(mangled_name)
    }

    fn instantiate_function(&mut self, base_name: &str, type_args: &[Type]) -> Result<String, CompileError> {
        let (origin_decl, origin_mod) = if let Some((decl, mod_name)) = self.generic_functions.get(base_name) { (decl.clone(), mod_name.clone()) }
        else {
             let found = self.generic_functions.iter().find(|(k, _)| base_name.ends_with(*k));
             if let Some((_, val)) = found { val.clone() } else { return Err(self.cur_error(&format!("Generic function '{}' not found", base_name))); }
        };
        let mangled_name = self.mangle_name(&format!("{}_{}", origin_mod, origin_decl.name), type_args);
        if self.monomorphized_functions.contains(&mangled_name) { return Ok(mangled_name); }
        if origin_decl.generic_params.len() != type_args.len() { return Err(self.cur_error("Generic function expects more args")); }
        let mut mapping = HashMap::new();
        for (i, param) in origin_decl.generic_params.iter().enumerate() { mapping.insert(param.clone(), type_args[i].clone()); }
    ...
    fn instantiate_function_from_decl(&mut self, decl: FunctionDecl, mapping: &HashMap<String, Type>, struct_prefix: &str, origin_mod: &str) -> Result<(), CompileError> {
         let mangled_name = if !struct_prefix.is_empty() { format!("{}_{}", struct_prefix, decl.name) }
         else { let args: Vec<Type> = decl.generic_params.iter().map(|p| mapping.get(p).unwrap().clone()).collect(); self.mangle_name(&format!("{}_{}", origin_mod, decl.name), &args) };
         if self.functions.contains_key(&mangled_name) { return Ok(()); }
         ...
         self.instantiation_queue.push((origin_mod.to_string(), new_decl, mapping.clone()));
         Ok(())
    }
    ...
    fn substitute_block(&self, block: &mut Block, mapping: &HashMap<String, Type>) -> Result<(), CompileError> {
         for stmt in &mut block.statements { self.substitute_statement(stmt, mapping)?; }
         Ok(())
    }
    
    fn substitute_statement(&self, stmt: &mut Statement, mapping: &HashMap<String, Type>) -> Result<(), CompileError> {
        match stmt {
            Statement::Let { type_name, value, .. } => { if let Some(t) = type_name { *t = self.substitute_type(t, mapping); } if let Some(v) = value { self.substitute_expression(v, mapping)?; } }
            Statement::Expr(e) => self.substitute_expression(e, mapping)?,
            Statement::Assign { lvalue, value } => { self.substitute_expression(lvalue, mapping)?; self.substitute_expression(value, mapping)?; }
            Statement::If { condition, then_branch, else_branch } => { self.substitute_expression(condition, mapping)?; self.substitute_block(then_branch, mapping)?; if let Some(b) = else_branch { self.substitute_block(b, mapping)?; } }
            Statement::While { condition, body } => { self.substitute_expression(condition, mapping)?; self.substitute_block(body, mapping)?; }
            Statement::Return(opt) => { if let Some(e) = opt { self.substitute_expression(e, mapping)?; } }
            Statement::Block(b) => self.substitute_block(b, mapping)?,
            Statement::When { subject, arms } => { self.substitute_expression(subject, mapping)?; for arm in arms { self.substitute_block(&mut arm.body, mapping)?; } }
    ...
    fn substitute_expression(&self, expr: &mut Expression, mapping: &HashMap<String, Type>) -> Result<(), CompileError> {
        match expr {
            Expression::Call { type_args, args, function, .. } => { for t in type_args { *t = self.substitute_type(t, mapping); } for a in args { self.substitute_expression(a, mapping)?; } self.substitute_expression(function, mapping)?; }
            Expression::StructInit { type_args, fields, .. } => { for t in type_args { *t = self.substitute_type(t, mapping); } for (_, e) in fields { self.substitute_expression(e, mapping)?; } }
            Expression::Binary { left, right, .. } => { self.substitute_expression(left, mapping)?; self.substitute_expression(right, mapping)?; }
            Expression::Unary { right, .. } => { self.substitute_expression(right, mapping)?; }
            Expression::MemberAccess { object, .. } => { self.substitute_expression(object, mapping)?; }
            Expression::Cast { expression, target_type } => { self.substitute_expression(expression, mapping)?; *target_type = self.substitute_type(target_type, mapping); }
            Expression::ArrayRepeat { value, .. } => { self.substitute_expression(value, mapping)?; }
            Expression::Index { left, index } => { self.substitute_expression(left, mapping)?; self.substitute_expression(index, mapping)?; }
            Expression::Sizeof(t) => { *t = self.substitute_type(t, mapping); }
            _ => {}
        }
        Ok(())
    }
        ...
        match typ {
            Type::Custom(name, args) => {
                if args.is_empty() {
                    ...
                    if self.structs.contains_key(&mangled) { return Type::Custom(mangled, args); }
                    
                    if self.structs.contains_key(&name) { return Type::Custom(name, args); }

                    if let Some(pos) = name.find('.') {
                        let alias = &name[..pos]; let sname = &name[pos+1..];
    ...
    pub fn analyze_main(&mut self, main_path: PathBuf) -> Result<Vec<TModule>, CompileError> { self.analyze_module_recursive(main_path)?; Ok(self.analyzed_modules.values().cloned().collect()) }

    fn analyze_module_recursive(&mut self, path: PathBuf) -> Result<String, CompileError> {
        let abs_path = fs::canonicalize(&path).map_err(|e| self.cur_error(&format!("Path error: {} ({})", e, path.display())))?;
        let mod_name = abs_path.file_stem().unwrap().to_string_lossy().to_string();
        
        if self.analyzed_modules.contains_key(&mod_name) { return Ok(mod_name); }
        if self.processing_stack.contains(&abs_path) { return Err(self.cur_error(&format!("Circular import detected: {}", abs_path.display()))); }
        
        self.processing_stack.insert(abs_path.clone());
        self.parsed_files.insert(abs_path.clone());

        let source = fs::read_to_string(&abs_path).map_err(|e| self.cur_error(&format!("Read error: {}", e)))?;
        ...
        for item in program.items {
            match item {
                Item::Import { path: imp_path, alias } => {
                    ...
                    if let Some(target_path) = found_path {
                        ...
                        self.current_imports.insert(alias.unwrap_or(target_mod_name.clone()), target_mod_name);
                    } else {
        ...
        for item in &raw_items {
            match item {
                ...
                Item::ExternFunction(f) => {
                    ...
                    self.functions.insert(f.name.clone(), (pts, f.return_type.clone(), Visibility::Public, false, "extern".into()));
                }
                Item::NativeFunction(f) => {
                    let pts: Vec<Type> = f.params.iter().map(|(_, t, _)| t.clone()).collect();
                    self.functions.insert(f.name.clone(), (pts.clone(), f.return_type.clone(), f.visibility.clone(), false, "native".into()));
                    if f.visibility == Visibility::Public { exports.insert(f.name.clone(), Symbol { name: f.name.clone(), typ: Type::Function(pts, Box::new(f.return_type.clone())), is_mutable: false, visibility: Visibility::Public }); }
                }
                Item::Impl(i) => {
        ...
        for item in raw_items {
            match item {
                Item::Impl(i) => { if i.generic_params.is_empty() { for mut m in i.methods { m.name = format!("{}_{}", i.struct_name, m.name); t_items.push(TItem::Function(self.analyze_function(m)?)); } } }
                Item::Function(f) => { if f.generic_params.is_empty() { t_items.push(TItem::Function(self.analyze_function(f)?)); } }
                Item::ExternFunction(f) => { t_items.push(TItem::ExternFunction(TExternFunctionDecl { name: f.name, params: f.params, return_type: f.return_type })); }
        ...
        for item in &t_items {
            match item {
                TItem::Struct(s) => { final_structs.insert(s.name.clone(), s.clone()); }
                TItem::Enum(e) => { final_enums.insert(e.name.clone(), e.clone()); }
                _ => {}
            }
        }
    ...
    fn resolve_qualified_identifier(&self, name: &str) -> Result<Symbol, CompileError> {
        if let Some(pos) = name.find('.') {
            ...
            if let Some(real_mod) = self.current_imports.get(alias) {
                let target_mod = self.analyzed_modules.get(real_mod).ok_or_else(|| self.cur_error("Module not analyzed"))?;
                if let Some(sym) = target_mod.exports.get(member) { return Ok(sym.clone()); }
                return Err(self.cur_error(&format!("Module '{}' does not export '{}'", alias, member)));
            }
        }
        
        // Priority 1: Local Scope (Shadowing)
        if let Some(sym) = self.current_scope.borrow().lookup(name) { return Ok(sym); }
        
        // Priority 2: Global Functions (already mangled)
        if let Some((pts, ret, vis, _, _)) = self.functions.get(name) { 
            return Ok(Symbol { name: name.to_string(), typ: Type::Function(pts.clone(), Box::new(ret.clone())), is_mutable: false, visibility: vis.clone() }); 
        }
        ...
        if name.starts_with(&prefix) {
            ...
            if let Some((pts, ret, vis, _, _)) = self.functions.get(stripped) {
                 return Ok(Symbol { name: stripped.to_string(), typ: Type::Function(pts.clone(), Box::new(ret.clone())), is_mutable: false, visibility: vis.clone() });
            }
        }
        ...
        if !name.starts_with(&prefix) {
            ...
            if let Some((pts, ret, vis, _, _)) = self.functions.get(&current_mangled) { return Ok(Symbol { name: current_mangled, typ: Type::Function(pts.clone(), Box::new(ret.clone())), is_mutable: false, visibility: vis.clone() }); }
        }
    ...
    fn analyze_function(&mut self, f: FunctionDecl) -> Result<TFunctionDecl, CompileError> {
        let old_pure = self.in_pure_context; self.in_pure_context = f.is_pure;
        ...
        for (name, typ, is_mut) in f.params {
            ...
            self.current_scope.borrow_mut().insert(name.clone(), Symbol { name: name.clone(), typ: mt.clone(), is_mutable: is_mut, visibility: Visibility::Private });
        ...
        Ok(TFunctionDecl { name: mangled, params: typed_params, return_type: ret_type, body, visibility: f.visibility, is_pure: f.is_pure })
    }

    fn analyze_block(&mut self, block: Block) -> Result<TBlock, CompileError> {
        let mut stmts = Vec::new();
        for s in block.statements { stmts.push(self.analyze_statement(s)?); }
        Ok(TBlock { statements: stmts })
    }

    fn analyze_statement(&mut self, stmt: Statement) -> Result<TStatement, CompileError> {
        match stmt {
            Statement::Let { name, is_mutable, type_name, value } => {
                let te = if let Some(e) = value { Some(self.analyze_expression(e)?) } else { None };
                let typ = if let Some(t) = type_name { self.mangle_type(t) } else { te.as_ref().unwrap().typ.clone() };
                self.current_scope.borrow_mut().insert(name.clone(), Symbol { name: name.clone(), typ: typ.clone(), is_mutable, visibility: Visibility::Private });
                Ok(TStatement::Let { name, var_type: typ, value: te })
            }
            Statement::Expr(e) => Ok(TStatement::Expr(self.analyze_expression(e)?)),
            Statement::Assign { lvalue, value } => Ok(TStatement::Assign { lvalue: self.analyze_expression(lvalue)?, rvalue: self.analyze_expression(value)? }),
            Statement::If { condition, then_branch, else_branch } => Ok(TStatement::If { condition: self.analyze_expression(condition)?, then_block: self.analyze_block(then_branch)?, else_block: else_branch.map(|b| self.analyze_block(b)).transpose()? }),
            Statement::While { condition, body } => Ok(TStatement::While { condition: self.analyze_expression(condition)?, body: self.analyze_block(body)? }),
            Statement::Return(oe) => Ok(TStatement::Return(oe.map(|e| self.analyze_expression(e)).transpose()?)),
            Statement::Throw { value } => Ok(TStatement::Throw { value: self.analyze_expression(value)? }),
            ...
            Statement::TryCatch { try_block, catch_variable, catch_block } => {
                ...
                self.enter_scope();
                self.current_scope.borrow_mut().insert(catch_variable.clone(), Symbol { name: catch_variable.clone(), typ: Type::String, is_mutable: false, visibility: Visibility::Private });
                let t_catch = self.analyze_block(catch_block)?;
                self.exit_scope();
                Ok(TStatement::TryCatch { try_block: t_try, catch_variable, catch_block: t_catch })
            }
            Statement::When { subject, arms } => {
                ...
                for arm in arms {
                    ...
                    let t_pat = match arm.pattern {
                        crate::ast::WhenPattern::Literal(l) => TWhenPattern::Literal(self.analyze_expression(l)?),
                        crate::ast::WhenPattern::Else => TWhenPattern::Else,
                        crate::ast::WhenPattern::EnumVariant { enum_name, variant_name, binding } => {
                             let mangled_enum = if let Some(pos) = enum_name.find('.') {
                                let alias = &enum_name[..pos]; let real_mod = self.current_imports.get(alias).ok_or_else(|| self.cur_error("Module not found"))?;
                                format!("{}_{}", real_mod, &enum_name[pos+1..])
                            } else { format!("{}_{}", self.current_module_name, enum_name) };
                            let mut tag = 0;
                                ...
                                if let Some(pos) = ed.variants.iter().position(|v| v.name == variant_name) {
                                    ...
                                    if let Some(b_name) = &binding {
                                        if let Some(v_type) = &ed.variants[pos].typ {
                                            self.current_scope.borrow_mut().insert(b_name.clone(), Symbol { name: b_name.clone(), typ: v_type.clone(), is_mutable: false, visibility: Visibility::Private });
                                            actual_binding = Some(b_name.clone());
                                        }
                                    }
                                }
                            }
                            TWhenPattern::EnumVariant { enum_name: mangled_enum, variant_name, binding: actual_binding, tag }
                        }
                        _ => TWhenPattern::Else,
                    };
                    let t_body = self.analyze_block(arm.body)?;
                    self.exit_scope();
                    t_arms.push(TWhenArm { pattern: t_pat, body: t_body });
                }
                Ok(TStatement::When { subject: t_subj, arms: t_arms })
            }
        ...
        match expr {
            ...
            Expression::Call { function, args, type_args } => {
                if let Expression::MemberAccess { object, field } = *function.clone() {
                    ...
                    // Static Method Check
                    if let Expression::MemberAccess { object: mod_obj, field: struct_name } = inner_obj.clone() {
                        if let Expression::Identifier(alias) = *mod_obj {
                            if let Some(real_mod) = self.current_imports.get(&alias).cloned() {
                                let base_name = format!("{}_{}", real_mod, struct_name);
                                let concrete_name = if !extracted_targs.is_empty() {
                                    let mut mangled_args = Vec::new();
                                    for t in &extracted_targs { mangled_args.push(self.mangle_type(t.clone())); }
                                ...
                                if let Some((pts, ret, _, _, _)) = self.functions.get(&mangled_method).cloned() {
                                    let mut tas = Vec::new(); for a in args { tas.push(self.analyze_expression(a)?); }
                                    return Ok(TExpression { kind: TExpressionKind::Call { function: mangled_method, args: tas }, typ: ret });
                                }
                            }
                        }
                    }

                    // Static Method Check on Identifier (Imported/Local Struct)
                    if let Expression::Identifier(struct_name) = inner_obj.clone() {
                        if let Ok(sym) = self.resolve_qualified_identifier(&struct_name) {
                             let is_type = match &sym.typ { Type::Custom(_,_) | Type::Enum(_,_) => true, _ => false };
                             if is_type {
                    ...
                    if let Ok(tobj) = self.analyze_expression(inner_obj.clone()) {
                        if let Type::Custom(sn, struct_args) = &tobj.typ {
                             let concrete_sn = if !struct_args.is_empty() { self.mangle_name(sn, struct_args) } else { sn.clone() };
                             let mangled_method = format!("{}_{}", concrete_sn, field);
                             if self.functions.contains_key(&mangled_method) {
                                 let mut tas = vec![tobj]; for a in args { tas.push(self.analyze_expression(a)?); }
                                 let ret_typ = if let Some((_, ret, _, is_pure, _)) = self.functions.get(&mangled_method) { if self.in_pure_context && !is_pure { return Err(self.cur_error("Pure call impure")); } ret.clone() } else { Type::Void };
                                 return Ok(TExpression { kind: TExpressionKind::Call { function: mangled_method, args: tas }, typ: ret_typ });
                             }
                             
                             // Fallback to local mangling
                             let local_mangled = format!("{}_{}_{}", self.current_module_name, concrete_sn, field);
                             if self.functions.contains_key(&local_mangled) {
                                 let mut tas = vec![tobj]; for a in args { tas.push(self.analyze_expression(a)?); }
                                 let ret_typ = if let Some((_, ret, _, is_pure, _)) = self.functions.get(&local_mangled) { if self.in_pure_context && !is_pure { return Err(self.cur_error("Pure call impure")); } ret.clone() } else { Type::Void };
                                 return Ok(TExpression { kind: TExpressionKind::Call { function: local_mangled, args: tas }, typ: ret_typ });
                             }
                        }
                    }
                }
                ...
                let (mut fnm, mut ret_typ) = match &fe.typ { Type::Function(_, ret) => { if let TExpressionKind::Identifier(n) = &fe.kind { (n.clone(), (**ret).clone()) } else { return Err(self.cur_error("Invalid call")); } } _ => { if let TExpressionKind::Identifier(n) = &fe.kind { (n.clone(), Type::Void) } else { return Err(self.cur_error("Invalid call")); } } };
                if !type_args.is_empty() {
                    let mut mangled_args = Vec::new(); for t in &type_args { mangled_args.push(self.mangle_type(t.clone())); }
                    let simple_name = if fnm.starts_with(&format!("{}_", self.current_module_name)) { &fnm[self.current_module_name.len()+1..] } else { &fnm };
                    let concrete_name = self.instantiate_function(simple_name, &mangled_args)?;
                    self.process_instantiation_queue()?; fnm = concrete_name;
                    if let Some((_, ret, _, is_pure, _)) = self.functions.get(&fnm) { if self.in_pure_context && !is_pure { return Err(self.cur_error("Pure call impure")); } ret_typ = ret.clone(); }
                } else if let Some((_, _, _, is_pure, _)) = self.functions.get(&fnm) { if self.in_pure_context && !is_pure { return Err(self.cur_error("Pure call impure")); } }
                let mut tas = Vec::new(); for a in args { tas.push(self.analyze_expression(a)?); }
                Ok(TExpression { kind: TExpressionKind::Call { function: fnm, args: tas }, typ: ret_typ })
            }
            Expression::Int(i) => Ok(TExpression { kind: TExpressionKind::Int(i), typ: Type::Int }),
            Expression::Float(f) => Ok(TExpression { kind: TExpressionKind::Float(f), typ: Type::Float }),
            Expression::String(s) => Ok(TExpression { kind: TExpressionKind::String(s), typ: Type::String }),
            Expression::Boolean(b) => Ok(TExpression { kind: TExpressionKind::Boolean(b), typ: Type::Bool }),
            Expression::Identifier(n) => {
                let sym = self.resolve_qualified_identifier(&n)?;
                Ok(TExpression { kind: TExpressionKind::Identifier(sym.name), typ: sym.typ })
            }
            Expression::Binary { left, operator, right } => {
                let tl = self.analyze_expression(*left)?; let tr = self.analyze_expression(*right)?;
                Ok(TExpression { kind: TExpressionKind::Binary { left: Box::new(tl.clone()), operator, right: Box::new(tr) }, typ: tl.typ })
            }
            Expression::Unary { operator, right } => {
                let tr = self.analyze_expression(*right)?;
                Ok(TExpression { kind: TExpressionKind::Unary { operator, right: Box::new(tr.clone()) }, typ: tr.typ })
            }
            Expression::MemberAccess { object, field } => {
                let (inner_obj, extracted_targs) = if let Expression::GenericSpecialization { expression, type_args } = *object.clone() { (*expression, type_args) } else { (*object.clone(), vec![]) };
                if let Expression::Identifier(alias) = inner_obj.clone() { if let Some(real_mod) = self.current_imports.get(&alias).cloned() { let target_mod = self.analyzed_modules.get(&real_mod).ok_or_else(|| self.cur_error("Module not analyzed"))?; if let Some(sym) = target_mod.exports.get(&field) { return Ok(TExpression { kind: TExpressionKind::Identifier(sym.name.clone()), typ: sym.typ.clone() }); } } }
            ...
            Expression::StructInit { name, fields, type_args } => {
                let base_name = if let Some(pos) = name.find('.') { let alias = &name[..pos]; let sname = &name[pos+1..]; let real_mod = self.current_imports.get(alias).ok_or_else(|| self.cur_error("Module not found"))?; format!("{}_{}", real_mod, sname) } else { format!("{}_{}", self.current_module_name, name) };
                let concrete_name = if !type_args.is_empty() { let mut mangled_args = Vec::new(); for t in &type_args { mangled_args.push(self.mangle_type(t.clone())); } let instantiated = self.instantiate_struct(&base_name, &mangled_args)?; self.process_instantiation_queue()?; instantiated } else { base_name };
                ...
                Ok(TExpression { kind: TExpressionKind::StructInit { name: concrete_name.clone(), fields: tfs }, typ: Type::Custom(concrete_name, vec![]) })
            }
            Expression::Index { left, index } => { let tl = self.analyze_expression(*left)?; let ti = self.analyze_expression(*index)?; match tl.typ.clone() { Type::Array(inner, _) | Type::Pointer(inner) => Ok(TExpression { kind: TExpressionKind::Index { array: Box::new(tl), index: Box::new(ti) }, typ: (*inner).clone() }), _ => Err(self.cur_error("Indexing non-array")) } }
            Expression::Cast { expression, target_type } => { let te = self.analyze_expression(*expression)?; let mt = self.mangle_type(target_type); Ok(TExpression { kind: TExpressionKind::Cast { expression: Box::new(te), target_type: mt.clone() }, typ: mt }) }
            Expression::Comptime { body } => { self.enter_scope(); let tbody = self.analyze_block(body)?; self.exit_scope(); let mut interpreter = Interpreter::new(); let res = interpreter.eval_block(&tbody); match res { Ok(Some(val)) => match val { Value::Int(i) => Ok(TExpression { kind: TExpressionKind::Int(i), typ: Type::Int }), Value::Float(f) => Ok(TExpression { kind: TExpressionKind::Float(f), typ: Type::Float }), Value::Bool(b) => Ok(TExpression { kind: TExpressionKind::Boolean(b), typ: Type::Bool }), Value::String(s) => Ok(TExpression { kind: TExpressionKind::String(s), typ: Type::String }), Value::Void => Ok(TExpression { kind: TExpressionKind::Null, typ: Type::Void }) }, Ok(None) => Ok(TExpression { kind: TExpressionKind::Null, typ: Type::Void }), Err(msg) => Err(self.cur_error(&msg)) } }
            ...
            Expression::EnumInit { enum_name, variant_name, value, type_args } => {
                ...
                let t_val = if let Some(v) = value { Some(Box::new(self.analyze_expression(*v)?)) } else { None };
                Ok(TExpression { kind: TExpressionKind::EnumInit { enum_name: mangled.clone(), variant_name, value: t_val, tag }, typ: Type::Enum(mangled, vec![]) })
            }
            Expression::Sizeof(t) => { let mt = self.mangle_type(t); Ok(TExpression { kind: TExpressionKind::Sizeof(mt), typ: Type::Int }) }
            _ => Ok(TExpression { kind: TExpressionKind::Null, typ: Type::Void })
        }
    }
}
