...
fn print_error_report(filename: &str, err: CompileError) {
    if let Ok(content) = fs::read_to_string(filename) {
        ...
        if err.line > 0 && err.line <= lines.len() {
            ...
            eprintln!("\x1b[1;31mError\x1b[0m in {}:{}:{}", filename, err.line, err.column);
            eprintln!("   |");
            eprintln!("{:2} | {}", err.line, line_content);
            eprintln!("   | \x1b[1;31m{}^\x1b[0m {}", " ".repeat(err.column), err.message);
            eprintln!("   |");
        } else {
...
fn build_file(filename: &str, lib_paths: Vec<PathBuf>) {
    ...
    match analyzer.analyze_main(PathBuf::from(filename)) {
        Ok(modules) => {
            ...
            cmd.arg(&obj_file).arg("libruntime.a").arg("-o").arg(&exe_file)
               .arg("-lm").arg("-lpthread").arg("-ldl");
            // Add user link args
            for arg in link_args { cmd.arg(arg); }
